    ...
    private static List<SpellTileType> makeAll() {
        ...
        all.add(make("branch_equals", (state, spell, tile) -> {
            ...
            if (r1[0] == r2[0] && r1[1] == r2[1] && r1[2] == r2[2] && r1[3] == r2[3]) {
                state.nextTile = state.curTile.add(Direction.fromId((int) (double) tile.properties.getOrDefault("branch", 0.0)).asVec2i(), new Vector2i());
            }
        ...
        all.add(make("get_collided", (state, spell, tile) -> {
            state.setRegisterBoolean(true, spell.collided);
        }, List.of()));
        return all;
    }
